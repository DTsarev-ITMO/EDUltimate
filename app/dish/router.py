...
from app.dish.schemas import ResponseDishGet, ResponseFoodAdd, ResponseFoodUpdate
from app.food.rb import RBFood
from app.users.dependencies import get_current_admin_user
from app.users.models import User

router = APIRouter(prefix='/food', tags=['Работа с рецептами'])

@router.get("/", summary="Получить все рецепты", response_model=list[ResponseDishGet])
async def get_all_dishes():
    return await DishDAO.find_all()
...
#
# @router.put("/update/")
# async def update(food: ResponseFoodUpdate, myself: User = Depends(get_current_admin_user)) -> dict:
...
# async def delete_food(food_name: str, myself: User = Depends(get_current_admin_user)) -> dict:
